...
typedef struct erow {
    ...
    int hl_open_comment; // Row had open comment at end in last syntax highligh check
} erow;
...
char *PY_HL_keywords[] = {
    ...
    "int|", "long|", "double|", "float|", "char|", "unsigned|", "signed|",
    "dict|", "list|", "set|", "frozenset|", "tuple|", NULL
};
...
char *JS_HL_keywords[] = {
    ...
    "typeof", "var", "void", "volatile", "while", "with", "yield", NULL
};
...
int editorRowRXtoCX(erow *row, int rx) {
    ...
    for (cx = 0; cx < row->size; cx++) {
        if (row->chars[cx] == '\t')
            cur_rx += (MUSE_TAB_STOP - 1) - (cur_rx % MUSE_TAB_STOP);
        cur_rx++;
...
void editorUpdateRow(erow *row) {
    ...
    free(row->render);
    row->render = malloc(row->size + tabs*(MUSE_TAB_STOP) + 1);

    int idx = 0;
    for (j = 0; j< row->size; j++) {
        if (row->chars[j] == '\t') {
            row->render[idx++] = ' ';
            while (idx % MUSE_TAB_STOP != 0) row->render[idx++] = ' ';
        } else {
